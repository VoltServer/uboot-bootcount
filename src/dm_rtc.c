 ...
 *        // as the I2C register offset.  So we use another property to specify the linux-nvram-offset:
 *        linux-nvram-offset = <0x00>;
...
static bool discover_dm_rtc(void)
{
    ...
    // if there is an nvmem-type property, use it later to select the correct nvmem device:
    char nvmem_type[64];
    int nvmem_type_len = dt_node_read_str(bc_node, "linux,nvmem-type", nvmem_type, sizeof(nvmem_type));
    if (nvmem_type_len > 0) {
        DEBUG_PRINTF(" found nvmem-type '%s'\n", nvmem_type);
    }
    ...
    // if there is a linux-nvram-offset property, use it instead of offset:
    uint32_t linux_nvram_offset = 0;
    if (dt_node_read_u32(bc_node, "linux,nvram-offset", &linux_nvram_offset)) {
        g_offset = (off_t)linux_nvram_offset;
        DEBUG_PRINTF(" found linux,nvram-offset 0x%lx\n", (unsigned long)g_offset);
    }
    ...
    while ((de = readdir(dev_dir))) {
        ...
        // if the dt definition included a nvmem-type property, check it matches the nvmem "type"
